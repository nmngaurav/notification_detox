...
class NotificationCardSystem(private val count: Int = 30) {
    ...
    fun init(width: Float, height: Float, appInfoList: List<aura.notification.filter.util.AppInfoManager.AppInfo>? = null) {
        cards.clear()
        ...
        val dataSource = if (appInfoList != null && appInfoList.isNotEmpty()) {
            ...
            val urgentCards = if (realUrgentApps.isNotEmpty()) {
                ...
                listOf(
                    Triple(NotificationType.UTILITY, 
                        aura.notification.filter.util.AppInfoManager.AppInfo("Bank Alert", null, ""),
                        Pair("Transaction Alert", "$500.00 debited")),
                    Triple(NotificationType.UTILITY,
                        aura.notification.filter.util.AppInfoManager.AppInfo("Security", null, ""),
            ...
            templates.map { (type, data) ->
                ...
                Triple(type, aura.notification.filter.util.AppInfoManager.AppInfo(appName, null, ""), Pair(title, preview))
            }
        }
